...
#define LUMP_RERL 0x4C524552
#define LUMP_REDI 0x49444552
#define LUMP_NTRO 0x4F52544E
#define LUMP_DATA 0x41544144
#define LUMP_VBIB 0x42494256
#define VTEX_FLAG_CUBE_TEXTURE 0x10
...
} vtex_resource_header_t;

uint8_t* get_resource_offset(vtex_resource_header_t* resource_header) {
	return (uint8_t*) ( (uint8_t*)(&resource_header->resource_offset) + resource_header->resource_offset);
}
...
{
  vtex_header_t* header = (vtex_header_t*) data;
...
inline uint8_t ClampToUInt8(int32_t nValue)
{
    if (nValue > 255)
        return 255;
    else if (nValue < 0)
...
void decode_dxt5(uint8_t* data, int width, int height, uint8_t** rgba_rows)
{
  //vtex_header_t* header = (vtex_header_t*) data;
  //int framesize = ((header->width+3)/4) * ((header->height+3)/4) * (128/8);
  //int pos = filesize - (framesize*frame_offset);
...
void get_image(vtex_resource_header_t *resource_header, vtex_resource_vtex_t *vtex, int mipmapWidth, int mipmapHeight, uint8_t* data, int filesize, char *out_path)
{
	int entrySize = 0;
    uint8_t** rgba_rows = 0;
			printf("vtex_image_format: %i\n",
			vtex->vtex_image_format
			);
	switch (vtex->vtex_image_format) {
		case 1://DXT1
			break;
		case 2://DXT5
			printf("mipmapWidth: %i\n"
		    	"mipmapHeight: %i\n",
				mipmapWidth,
				mipmapHeight
				);
			rgba_rows = malloc(sizeof(uint8_t*)*mipmapHeight);
			for(int i = 0; i < mipmapHeight; ++i) {
				rgba_rows[i] = malloc(mipmapWidth * 4);
				memset(rgba_rows[i], 0x00, mipmapWidth * 4);
			}

			decode_dxt5(get_resource_offset(resource_header) + resource_header->resource_length, mipmapWidth, mipmapHeight, rgba_rows);


			//int nOverrideAlpha = 0;
            float offset = 128.0 / 255.0;
			for(int i = 0; i < mipmapHeight; ++i) {
				for(int j = 0; j < mipmapWidth; ++j) {
                    uint8_t *pixel = &rgba_rows[i][j*4];
					float r = pixel[0] / 255.0;
					float g = pixel[1] / 255.0;
					float b = pixel[2] / 255.0;
					float a = pixel[3] / 255.0;

					float Y = a;
					float scale = 1.0 / ((255.0 / 8.0) * (float)b + 1.0);
					float Co = ((float)r - offset) * scale;
					float Cg = ((float)g - offset) * scale;
					...
					pixel[0] = ClampToUInt8(y - co - cg);
					pixel[1] = ClampToUInt8(y + cg);
					pixel[2] = ClampToUInt8(y + co - cg);
					pixel[3] = (nOverrideAlpha != 0) ? nOverrideAlpha : y;*/

					pixel[0] = ClampToUInt8(R * 255.0);
					pixel[1] = ClampToUInt8(G * 255.0);
					pixel[2] = ClampToUInt8(B * 255.0);
					//pixel[0] = (scale * 255.0);
					//pixel[1] = (scale * 255.0);
					//pixel[2] = (scale * 255.0);
					pixel[3] = 255;
			...
			png_set_IHDR(png_ptr, info_ptr, mipmapWidth, mipmapHeight, 8,
		...
		case 17:
		case 18:
			entrySize = filesize - (get_resource_offset(resource_header) - data  + resource_header->resource_length);
			if (entrySize) {
				FILE* of = fopen(out_path, "wb");
				if(!of) {
					fprintf(stderr, "Could not open \"%s\" for writing\n", out_path);
		        ...
		        fwrite(get_resource_offset(resource_header) + resource_header->resource_length, 1, entrySize, of);
				fclose(of);
			}
			break;
		default:
			break;
			printf("unknown vtex_image_format: %i\n",
			vtex->vtex_image_format
			);
	}
	/*if (vtex->vtex_image_format == 16 || vtex->vtex_image_format == 17 || vtex->vtex_image_format == 18) {
		entrySize = filesize - (resource_header->resource_offset + resource_header->resource_length);
		entrySize = resource_header->resource_offset + resource_header->resource_length;
	...
	/*printf("entrySize: %i\n",
  		entrySize
			...
			/*file.seek(lump.offset + lump.length, os.SEEK_SET)

			outfilename = string.replace(self.fileName, '_png.vtex_c', '.png')
			outfilename = string.replace(outfilename, '.vtex_c', '')
			print outfilename
			f = open(outfilename, 'w')

			f.write(file.read(entrySize))
			f.close()
			return*/
}

void load_data_vtex(vtex_resource_header_t* resource_header, uint8_t* data, int filesize, char *out_path)
{
	vtex_resource_vtex_t *vtex = (vtex_resource_vtex_t*)(get_resource_offset(resource_header));

    int faceCount = 1;
	if ((vtex->vtex_flags & VTEX_FLAG_CUBE_TEXTURE) == VTEX_FLAG_CUBE_TEXTURE) {
		faceCount = 6;
	}

	//file.seek(self.fileLength);
	int mipmapWidth = vtex->vtex_width * pow(0.5, vtex->vtex_num_mipLevels - 1);
	int mipmapHeight = vtex->vtex_height * pow(0.5, vtex->vtex_num_mipLevels - 1);
	//uint8_t* imageData;

	// Only keep last (biggest) mipmap
    for(int resIndex = 0; resIndex < vtex->vtex_num_mipLevels; resIndex++) {
		//print 'Mipmap : ', resIndex, mipmapWidth, mipmapHeight
	//Todo : add frame support + depth support
		for(int faceIndex = 0; faceIndex < faceCount; faceIndex++) {
			...
			//self.getImage(file, lump, mipmapWidth, mipmapHeight, lump.imageFormat)
			//#lump.imageData[faceIndex] = str2ab(this.getImage(reader, mipmapWidth, mipmapHeight, lump.imageFormat));
...
int main(int argc, char** argv)
{
  ...
  struct argp argp = {arg_options, arg_parse_opt, "INPUT.vtex_c OUTPUT.png"};
  ...
  vtex_header_t* header = (vtex_header_t*)filedata;

	vtex_resource_header_t *resources[header->resource_count];

  uint8_t* resource_data = filedata + header->resource_offset + 8;
	for(int i = 0; i < header->resource_count; i++) {
		vtex_resource_header_t* resource_header = (vtex_resource_header_t*)(resource_data + sizeof(vtex_resource_header_t) * i);
		resources[i] = resource_header;

		if(options.verbose) {
			printf("resource_type: %i\n"
			"resource_offset: %i\n"
			"resource_length: %i\n",
			resource_header->resource_type,
			resource_header->resource_offset,
			resource_header->resource_length);
		}
	}

  for(int i = 0; i < header->resource_count; i++) {
    vtex_resource_header_t* resource_header = resources[i];
    switch (resource_header->resource_type) {
		case LUMP_RERL:
		case LUMP_REDI:
		case LUMP_NTRO:
		case LUMP_VBIB:
			break;
		case LUMP_DATA:
        	load_data_vtex(resource_header, filedata, filesize, options.out_path);
  ...
  /*if(header->version[0] > 7
     || (header->version[0] == 7 && header->version[1] > 5)) {
    fprintf(stderr, "Unsupported VTEX file version: %i.%i\n",
